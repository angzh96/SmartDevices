<template>
  <div class="app-container">
    <!-- selectbar -->
    <div class="selectbar">
      <span style="font-size:15px">设备类型：</span>
    ...
    >
      ...
      <el-table-column label="控制端口" min-width="100" align="center">
        <template slot-scope="scope">
          {{scope.row.cPort}}
        </template>
      </el-table-column>
       <el-table-column label="环境数据端口" min-width="100" align="center">
        <template slot-scope="scope">
          {{scope.row.ePort}}
        </template>
      </el-table-column>
       <el-table-column label="视频数据端口" min-width="100" align="center">
        <template slot-scope="scope">
          {{scope.row.vPort}}
        </template>
      </el-table-column>
      ...
      <div class="leftbutton">
        ...
        <el-dialog title="添加设备信息" :visible.sync="dialogFormVisible" width="35%" center>
          ...
          <el-form ref="insertForm" :model="form" :label-position="formLabelPosition" :rules="insertRules" style="margin: 20px 20px -30px 20px;">
            <el-form-item label="设备类型：" :label-width="formLabelWidth" prop="dValue">
            <el-select v-model="form.dValue" size="small" placeholder="请选择" >
            <el-option
              ...
              <el-form-item label="设备编号：" :label-width="formLabelWidth" prop="deviceID">
                <el-input v-model="form.deviceID" placeholder="请输入设备编号" size="'mini'" ref="deviceID" name="deviceID"></el-input>
              </el-form-item>
              ...
              <el-form-item label="控制端口：" :label-width="formLabelWidth" prop="cPort">
                <el-input v-model="form.cPort" placeholder="请输入小车控制的tcp端口" ref="cPort"></el-input>
              </el-form-item>
              <!--el-form-item label="链接测试：" :label-width="formLabelWidth" prop="isConnect">
                <el-button plain @click="testCount">连接测试</el-button>
              </el-form-item-->
              <el-form-item label="环境数据端口：" :label-width="formLabelWidth" prop="ePort">
                <el-input v-model="form.ePort" placeholder="请输入接收小车环境数据的udp端口"></el-input>
              </el-form-item>
            <el-form-item label="视频数据端口：" :label-width="formLabelWidth" prop="vPort">
              <el-input v-model="form.vPort" placeholder="请输入接收小车视频数据的udp端口"></el-input>
            </el-form-item>
          </el-form>
          <div slot="footer" class="dialog-footer">
            <el-button @click="cancelCar">取 消</el-button>
            <el-button type="primary" @click="insertDevice">确 定</el-button>
          </div>
        </el-dialog>
      </div>
      <!-- rightbutton -->
...
export default {
  ...
  data() {
    ...
    const validateCport =(rule, value, callback) => {
      if(!value) {
        callback(new Error('控制端口不能为空！'))
      }
      else{
        callback()
      }
    ...
    const validateVport =(rule, value, callback) => {
      if(!value) {
        callback(new Error('视频数据端口不能为空！'))
      }
      else{
        callback()
      }
    ...
    const validateEport =(rule, value, callback) => {
      if(!value) {
        callback(new Error('环境数据端口不能为空！'))
      }
      else{
        callback()
      }
    ...
    // const validatedValue =(rule, value, callback) => {
    //   if(!value) {
    ...
    return {
      ...
      form: {deviceID: '',type: '',owner: '',cPort: '',ePort: '',vPort: '',dValue: ''},
      insertRules: {
        deviceID: [{required: true, trigger: 'blur', validator: validateCarID}],
        type: [{required: true, trigger: 'blur', validator: validateType}],
        owner: [{required: true, trigger: 'blur', validator: validateOwner}],
        cPort: [{required: true, trigger: 'blur', validator: validateCport}],
        ePort: [{required: true, trigger: 'blur', validator: validateEport}],
        vPort: [{required: true, trigger: 'blur', validator: validateVport}],
        // dValue: [{required: true, trigger: 'blur', validator: validatedValue}]
      },
  ...
  methods: {
    ...
    getCarData(){
      ...
      if (type == 0){
        ...
        let url = process.env.VUE_APP_BASE_API + '/device/UAV/allUAVData'
        console.log(url);
    ...
    insertDevice(){
      var deviceType = this.form.dValue;
    ...
    insertUAV() {
      this.$refs.insertForm.validate(valid => {
        if (valid) {
          this.$ajax.post(process.env.VUE_APP_BASE_API + '/device/UAV/insertUAV',{
          deviceID: this.form.deviceID,
          type: this.form.type,
          owner: this.form.owner,
          cPort: this.form.cPort,
          ePort: this.form.ePort,
          vPort: this.form.vPort,
          dValue: this.form.dValue
        }).then( res => {
          console.log(res)
          if (res.data == 1){
            this.$message({
    ...
    saveInfo(row) {
      ...
      if (this.value == 0){
        this.$ajax({
            url: process.env.VUE_APP_BASE_API + '/device/UAV/updateUAV',
            method: 'post',
